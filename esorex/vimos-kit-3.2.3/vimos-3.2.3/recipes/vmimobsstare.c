...
static cxint vmimobsstare(PilSetOfFrames *);
...
static cxint
vmimobsstare_create(cpl_plugin *plugin)
{
...
static cxint
vmimobsstare_exec(cpl_plugin *plugin)
{
    ...
    if (vmimobsstare(sof) == EXIT_SUCCESS) {
        ...
        status = vmCplPostProcessFrames(sof, "vmimobsstare");
...
static cxint
vmimobsstare_destroy(cpl_plugin *plugin)
{
...
static cxint
vmimobsstare(PilSetOfFrames *sof)
{
...
*/
  ...
  if (computeQC) {
    ...
    */
    ...
    if (readStringDescriptor(rawImage->descs,
      ...
      cpl_msg_error(task, "String descriptor %s not found",
                  pilTrnGetKeyword("FilterId", quadrant));
  ...
  if (astFrame)
    pilFrmSetType(astFrame, PIL_FRAME_TYPE_RAW);
    ...
    if (nstars < minstars) {
      cpl_msg_warning(task, "Less than %d stars in Galaxy Table (%d): "
        ...
        while (excluded > 0 && nIter < maxIter) {
          ...
          average = newAverage;
          j -= excluded;
          cpl_msg_debug(task,"Excluded       : %d", excluded);
          nIter++;
        }
