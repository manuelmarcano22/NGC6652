...
const char *vimoswcsset_errmsg[] = {
   0,
   "Inconsistent or unrecognized coordinate axis types"};

const char *vimoswcsfwd_errmsg[] = {
   0,
...
int
vimoswcsset (naxis, ctype, vimoswcs)

const int naxis;
const char ctype[][9];
struct vimoswcsprm *vimoswcs;

{
   ...
   if (strcmp(vimoswcs->pcode, "")) {
      vimoswcs->flag = VIMOSWCSSET;
   } else {
...
int
vimoswcsfwd(ctype, vimoswcs, world, crval, cel, phi, theta, prj, imgcrd, lin, pixcrd)
...

{
   ...
   if (vimoswcs->flag != VIMOSWCSSET) {
      if (vimoswcsset(lin->naxis, ctype, vimoswcs)) return 1;
   }

   /* Convert to relative physical coordinates. */
   for (j = 0; j < lin->naxis; j++) {
      if (j == vimoswcs->lng) continue;
      if (j == vimoswcs->lat) continue;
      imgcrd[j] = world[j] - crval[j];
   }

   if (vimoswcs->flag != 999) {
      /* Compute projected coordinates. */
      if (strcmp(vimoswcs->pcode, "NCP") == 0) {
         /* Convert NCP to SIN. */
         if (cel->ref[2] == 0.0) {
            return 2;
         }
      ...
      if ((err = celfwd(vimoswcs->pcode, world[vimoswcs->lng], world[vimoswcs->lat], cel,
                   phi, theta, prj, &imgcrd[vimoswcs->lng], &imgcrd[vimoswcs->lat]))) {
         return err;
      }
      ...
      if (vimoswcs->cubeface != -1) {
         ...
         if (imgcrd[vimoswcs->lat] < -0.5*offset) {
            imgcrd[vimoswcs->lat] += offset;
            imgcrd[vimoswcs->cubeface] = 5.0;
         } else if (imgcrd[vimoswcs->lat] > 0.5*offset) {
            imgcrd[vimoswcs->lat] -= offset;
            imgcrd[vimoswcs->cubeface] = 0.0;
         } else if (imgcrd[vimoswcs->lng] > 2.5*offset) {
            imgcrd[vimoswcs->lng] -= 3.0*offset;
            imgcrd[vimoswcs->cubeface] = 4.0;
         } else if (imgcrd[vimoswcs->lng] > 1.5*offset) {
            imgcrd[vimoswcs->lng] -= 2.0*offset;
            imgcrd[vimoswcs->cubeface] = 3.0;
         } else if (imgcrd[vimoswcs->lng] > 0.5*offset) {
            imgcrd[vimoswcs->lng] -= offset;
            imgcrd[vimoswcs->cubeface] = 2.0;
         } else {
            imgcrd[vimoswcs->cubeface] = 1.0;
         }
      }
   }
...
int
vimoswcsrev(ctype, vimoswcs, pixcrd, lin, imgcrd, prj, phi, theta, crval, cel, world)
...

{
   ...
   if (vimoswcs->flag != 999) {
      /* Do we have a CUBEFACE axis? */
      if (vimoswcs->cubeface != -1) {
         face = (int)(imgcrd[vimoswcs->cubeface] + 0.5);
         if (fabs(imgcrd[vimoswcs->cubeface]-face) > 1e-10) {
            return 3;
         }
         ...
         case 2:
            ...
            break;
         case 3:
            imgcrd[vimoswcs->lng] += offset*2;
            break;
         case 4:
            imgcrd[vimoswcs->lng] += offset*3;
            break;
         case 5:
            imgcrd[vimoswcs->lat] -= offset;
            break;
      ...
      if ((err = celrev(vimoswcs->pcode, imgcrd[vimoswcs->lng], imgcrd[vimoswcs->lat], prj,
                   phi, theta, cel, &world[vimoswcs->lng], &world[vimoswcs->lat]))) {
         return err;
      }
   }
...
int
vimoswcsmix(ctype, vimoswcs, mixpix, mixcel, vspan, vstep, viter, world, crval, cel,
...

{
   ...
   for (istep = 0; istep <= nstep; istep++) {
      ...
      if (mixcel == 1) {
         ...
         lat0 = span[0];
         world[vimoswcs->lat] = lat0;
         if ((err = vimoswcsfwd(ctype, vimoswcs, world, crval, cel, phi, theta, prj,
                          imgcrd, lin, pixcrd))) {
            return err;
         }
         ...
         for (retry = 0; retry < 4; retry++) {
            ...
            while (lat0 > span[0]) {
               ...
               world[vimoswcs->lat] = lat0;
               if ((err = vimoswcsfwd(ctype, vimoswcs, world, crval, cel, phi, theta,
   ...
   *theta = -90.0;
   for (j = 1; j <= 2; j++) {
      ...
      *theta = -*theta;
      ...
      if (mixcel == 1) {
         if (fabs(fmod(world[vimoswcs->lng]-lng,360.0)) > tol) continue;
         if (lat < span[0]) continue;
         if (lat > span[1]) continue;
         world[vimoswcs->lat] = lat;
      } else {
         if (fabs(world[vimoswcs->lat]-lat) > tol) continue;
      ...
      lng = world[vimoswcs->lng];
      lat = world[vimoswcs->lat];

      /* Feed native coordinates to vimoswcsfwd() with cel0 set to unity. */
      world[vimoswcs->lng] = -180.0;
      world[vimoswcs->lat] = *theta;
      if ((err = vimoswcsfwd(ctype, vimoswcs, world, crval, &cel0, phi, theta, prj,
                       imgcrd, lin, pixcrd))) {
         return err;
      }
      d0 = pixcrd[mixpix] - pixmix;
      ...
      for (iter = 1; iter <= niter; iter++) {
         ...
         world[vimoswcs->lng] = phi0 + lambda*(phi1 - phi0);
         if ((err = vimoswcsfwd(ctype, vimoswcs, world, crval, &cel0, phi, theta, prj,
                          imgcrd, lin, pixcrd))) {
            return err;
         }
         ...
         if (signb (d0) == signb (d)) {
            phi0 = world[vimoswcs->lng];
            d0 = d;
         } else {
            phi1 = world[vimoswcs->lng];
            d1 = d;
         }
      }
   }
 ...
 * Dec 20 1999	Doug Mink - Include vimoswcslib.h, which includes wcs.h, wcstrig.h
 *
