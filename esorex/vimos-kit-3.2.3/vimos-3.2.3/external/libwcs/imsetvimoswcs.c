...
extern int FitMatch();
extern int FitPlate();
extern int FluxSortStars(double *sx, double  *sy, double  *sb, int *sc, int ns);
extern struct WorldCoor *GetFITSVIMOSWCS ();
extern char *getimcat();
extern void SetFITSVIMOSWCS();
extern void fk524e();
extern int iscdfit();
void setsecpix2 (double secpix);
void setsecpix (double secpix);
void setrefpix (double x, double y);
void setdcenter(double ra, double dec);
void setcd(double * cd);
void setsys(int comsys0);
void setrot(int rot);
...
static int fitvimoswcs = 1;			/* If 1, fit WCS, else use current WCS */
...
int
SetVIMOSWCSFITS (filename, header, image, refcatname, verbose)
...

{
    ...
    struct WorldCoor *vimoswcs=0;	/* WCS structure */
	...
	vimoswcs = GetFITSVIMOSWCS (filename,header,verbose,&cra,&cdec,&dra,&ddec,
			  &secpix, &imw,&imh,&refsys, &refeq);
	if (novimoswcs (vimoswcs)) {
	    ret = 0;
	    goto out;
	    }
	nbin = FitMatch (nbin, sx, sy, gra, gdec, vimoswcs, verbose);
	...
	vimoswcscstr (refcoor, refsys, refeq, refep);
	}

    /* get nominal position and scale */
getfield:
    vimoswcs = GetFITSVIMOSWCS (filename,header,verbose,&cra,&cdec,&dra,&ddec,&secpix,
    ...
    ng = ctgread (refcatname,refcat,0,cra,cdec,dra,ddec,0.0,refsys,refeq,
		  vimoswcs->epoch,mag1,mag2,ngmax,&starcat,
	...
	gx[ig] = 0.0;
	gy[ig] = 0.0;
	vimoswcs2pix (vimoswcs, gra[ig], gdec[ig], &gx[ig], &gy[ig], &goff[ig]);
	}
	...
	*/
	...
	if (verbose) {
	    ...
	    for (is = 0; is < nbs; is++) {
		pix2vimoswcs (vimoswcs, sx[is], sy[is], &ra, &dec);
	...
	nbin = StarMatch (nbs,sx,sy,nbg,gra,gdec,gx,gy,tolerance,vimoswcs,verbose);
...
match:
    ...
    if (verbose || !fitvimoswcs) {
	imcatname = getimcat ();
	if (vimoswcs->ncoeff1 > 0)
	    printf ("# %d-term x, %d-term y polynomial fit\n",
		    vimoswcs->ncoeff1, vimoswcs->ncoeff2);
	else
	    printf ("# Arcsec/Pixel= %.6f %.6f  Rotation= %.6f degrees\n",
		    3600.0*vimoswcs->xinc, 3600.0*vimoswcs->yinc, vimoswcs->rot);
	ra2str (rstr, 32, vimoswcs->xref, 3);
	dec2str (dstr, 32, vimoswcs->yref, 2);
	printf ("# Optical axis= %s  %s %s x= %.2f y= %.2f\n",
		rstr,dstr, refcoor, vimoswcs->xrefpix, vimoswcs->yrefpix);
	ra = vimoswcs->xref;
	dec = vimoswcs->yref;
	if (refsys == VIMOSWCS_J2000) {
	    (void)fk524e (&ra, &dec, vimoswcs->epoch);
	    ra2str (rstr, 32, ra, 3);
	    dec2str (dstr, 32, dec, 2);
	    printf ("# Optical axis= %s  %s B1950  x= %.2f y= %.2f\n",
		    rstr,dstr, vimoswcs->xrefpix, vimoswcs->yrefpix);
	    }
	else {
	    fk425e (&ra, &dec, vimoswcs->epoch);
	    ra2str (rstr, 32, ra, 3);
	    dec2str (dstr, 32, dec, 2);
    ...
    if (nmatch > 0) {
	int rprint = verbose || !fitvimoswcs;
	...
	PrintRes (header,vimoswcs,nmatch,sx1,sy1,gra1,gdec1,gm1,gnum1,refcat,rprint);
	...
	if (!iterate && !recenter && fitplate) {
	    ...
	    /* Fit residuals */
	    if (FitPlate (vimoswcs, sx1, sy1, gra1, gdec1, nmatch, fitplate,
			  verbose))
		...
		printf ("# nmatch= %d nstars= %d between %s and %s niter= %d\n",
			nmatch, nmax, refcatname, imcatname, niter);
		PrintRes (header,vimoswcs,nmatch,sx1,sy1,gra1,gdec1,gm1,gnum1,refcat,verbose);
	...
	if (refcatname == NULL)
	    fprintf (stderr, "SetVIMOSWCSFITS: No matches in %s:\n",
		     matchcat);
	else if (strlen (imcatname) == 0)
	    fprintf (stderr, "SetVIMOSWCSFITS: No matches between %s and %s:\n",
		     refcatname, filename);
	else
	    fprintf (stderr, "SetVIMOSWCSFITS: No matches between %s and %s:\n",
    ...
    if (iterate) {
	setdcenter (vimoswcs->xref, vimoswcs->yref);
	setsys (vimoswcs->sysvimoswcs);
	setrefpix (vimoswcs->xrefpix, vimoswcs->yrefpix);
	if (iscdfit())
	    setcd (vimoswcs->cd);
	else {
	    setsecpix (-3600.0 * vimoswcs->xinc);
	    setsecpix2 (3600.0 * vimoswcs->yinc);
	    setrot (vimoswcs->rot);
	    }
	iterate--;
	imfrac = 0.0;
    ...
    if (recenter) {
	double ra, dec, x, y;
	x = 0.5*vimoswcs->nxpix;
	y = 0.5*vimoswcs->nypix;
	pix2vimoswcs (vimoswcs, x, y, &ra, &dec);
	setdcenter (ra, dec);
	setsys (vimoswcs->sysvimoswcs);
	setrefpix (x, y);
	setsecpix (-3600.0 * vimoswcs->xinc);
...
struct WorldCoor *vimoswcs;	/* Image World Coordinate System */
...

{
    ...
    for (i = 0; i < nmatch; i++) {
	vimoswcs2pix (vimoswcs, gra1[i], gdec1[i], &gx, &gy, &goff);
	dx = gx - sx1[i];
	dy = gy - sy1[i];
	dx2 = dx * dx;
	dy2 = dy * dy;
	...
	pix2vimoswcs (vimoswcs, sx1[i], sy1[i], &sra, &sdec);
	sep = 3600.0 * vimoswcsdist(gra1[i],gdec1[i],sra,sdec);
