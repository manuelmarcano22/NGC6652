...
int
worldpos (xpix, ypix, vimoswcs, xpos, ypos)
...

{
  ...
  if (vimoswcs->rotmat) {
    tx = dx * vimoswcs->cd[0] + dy * vimoswcs->cd[1];
    dy = dx * vimoswcs->cd[2] + dy * vimoswcs->cd[3];
    dx = tx;
    }
  ...
  *ypos = yref + dy;
  ...
  switch (itype) {
    ...
    case VIMOSWCS_CAR:   /* -CAR Cartesian (was WCS_PIX pixel and WCS_LIN linear) */
      rat =  ra0 + l;
      dect = dec0 + m;
      break;

    case VIMOSWCS_SIN: /* -SIN sin*/ 
    ...
    case VIMOSWCS_TAN:   /* -TAN tan */
    case VIMOSWCS_TNX:   /* -TNX tan with polynomial correction */
      if (sins>1.0) return 1;
      dect = cos0 - m * sin0;
    ...
    case VIMOSWCS_NCP:   /* -NCP North celestial pole*/
      dect = cos0 - m * sin0;
      if (dect==0.0) return 1;
      rat = ra0 + atan2 (l, dect);
      dt = cos (rat-ra0);
    ...
    case VIMOSWCS_GLS:   /* -GLS global sinusoid */
      dect = dec0 + m;
    ...
    case VIMOSWCS_MER:   /* -MER mercator*/
    ...
    case VIMOSWCS_STG:   /* -STG Sterographic*/
    ...
    case VIMOSWCS_COE:    /* COE projection code from Andreas Wicenic, ESO */
      td = tan (dec0);
      y0 = 1.0 / td;
      mt = y0 - m;
      if (dec0 < 0.)
...
int
worldpix (xpos, ypos, vimoswcs, xpix, ypix)
...
/* Structure elements */
  ...
  switch (itype) {
    ...
    case VIMOSWCS_CAR:   /* -CAR Cartesian */
      l = ra - ra0;
      m = dec - dec0;
      break;
    ...
    case VIMOSWCS_COE:    /* allan: -COE projection added, AW, ESO*/
	gamby2 = sin (dec0);
	tthea = tan (dec0);
	rthea = 1. / tthea;
	a = -2. * tthea;
	...
	ansq = an * an;
	if (vimoswcs->rotmat)
	    l = rap * an * (1.0 - ansq/6.0) * (vimoswcs->cd[0] / fabs(vimoswcs->cd[0]));
	else
  ...
  /* Scale and rotate using CD matrix */
  if (vimoswcs->rotmat) {
    tx = dx * vimoswcs->dc[0] + dy * vimoswcs->dc[1];
    dy = dx * vimoswcs->dc[2] + dy * vimoswcs->dc[3];
    dx = tx;
    }
  ...
  /* Convert to pixels  */
  *xpix = dx + xrefpix;
  if (itype == VIMOSWCS_CAR) {
    if (*xpix > vimoswcs->nxpix) {
      x = *xpix - (360.0 / xinc);
      if (x > 0.0) *xpix = x;
      }
    else if (*xpix < 0) {
      x = *xpix + (360.0 / xinc);
      if (x <= vimoswcs->nxpix) *xpix = x;
      }
    }
  *ypix = dy + yrefpix;
