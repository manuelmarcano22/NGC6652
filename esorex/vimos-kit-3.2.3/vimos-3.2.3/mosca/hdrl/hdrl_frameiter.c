...
static void hdrl_frameiter_delete(void * state);
                   ...
                   intptr_t * dims)
{
    ...
    for (intptr_t i = 0; i < naxes; i++) {
        ...
        if (dims && dims[i] > 0) {
            ...
            state->dim[axes[i]] = state->dim[axes[i]];
...
static void get_data(hdrl_frameiter_state * state)
{
    if (state->naxes == 2 && state->naxes_max == HDRL_FRAMEITER_AXIS_NAXIS2) {
        if (state->axes[0] <= HDRL_FRAMEITER_AXIS_EXT &&
            ...
            const cpl_frame * frm =
                cpl_frameset_get_position_const(state->frames,
                                                state->pos[HDRL_FRAMEITER_AXIS_FRAME]);
            state->data.image =
                cpl_image_load(cpl_frame_get_filename(frm),
                               CPL_TYPE_UNSPECIFIED, 0,
                               state->pos[HDRL_FRAMEITER_AXIS_EXT]);
            state->data.plist =
                cpl_propertylist_load(cpl_frame_get_filename(frm),
                                      state->pos[HDRL_FRAMEITER_AXIS_EXT]);
            return;
        }
        else {
    ...
    else if (state->naxes == 1 && state->axes[0] == HDRL_FRAMEITER_AXIS_FRAME) {
    ...
    else if (state->naxes == 1 && state->axes[0] == HDRL_FRAMEITER_AXIS_EXT) {
        cpl_msg_debug(cpl_func, "Getting frame 0, ext %zd",
                     state->pos[HDRL_FRAMEITER_AXIS_EXT]);
        const cpl_frame * frm =
            cpl_frameset_get_position_const(state->frames, 0);
        state->data.image = cpl_image_load(cpl_frame_get_filename(frm),
                                           CPL_TYPE_UNSPECIFIED, 0,
                                           state->pos[HDRL_FRAMEITER_AXIS_EXT]);
        state->data.plist =
