...
hdrl_flat_test_case(hdrl_imagelist* imglist,
                    ...
                    const cpl_size fsx,
                    const cpl_size fsy,
                    const hdrl_parameter* collapse_method,
                    const int mask_sw,
                    const double val1,
                    const double err1,
                    const double val2,
...

{
    ...
    if(collapse_method == HDRL_COLLAPSE_MEAN) {
        cpl_msg_warning(cpl_func, "case collapse mean");
        collapse_params = hdrl_collapse_mean_parameter_create();
    } else if (collapse_method == HDRL_COLLAPSE_MEDIAN) {
        cpl_msg_warning(cpl_func,"case collapse median");
...
static cpl_error_code hdrl_flat_test_multi_options(void)
{
    ...
    double error_expected_pix1_mean = 0.;
    double error_expected_pix1_median = 0.;

    error_expected_pix1_mean=cpl_vector_get_sum(valserr_rel);
    error_expected_pix1_mean=sqrt(error_expected_pix1_mean)/nima;

    error_expected_pix1_median=error_expected_pix1_mean* sqrt(CPL_MATH_PI_2);
    ...
    hdrl_flat_test_case(imglist, rect_region_bad_area_def, HDRL_FLAT_FREQ_LOW,
                        filter_size_x, filter_size_y, HDRL_COLLAPSE_MEDIAN, 0,
    ...
    hdrl_flat_test_case(imglist, rect_region_bad_area_def, HDRL_FLAT_FREQ_LOW,
                        filter_size_x, filter_size_y, HDRL_COLLAPSE_MEDIAN, 1,
                        1, error_expected_pix1_median, r_median, 91.4844);
    ...
    hdrl_flat_test_case(imglist, rect_region_bad_area_def, HDRL_FLAT_FREQ_LOW,
                        filter_size_x, filter_size_y, HDRL_COLLAPSE_MEAN, 0,
    ...
    hdrl_flat_test_case(imglist, rect_region_bad_area_def, HDRL_FLAT_FREQ_LOW,
                        filter_size_x, filter_size_y, HDRL_COLLAPSE_MEAN, 1,
