                          ...
                          int swap)
{
    ...
    for (intptr_t i = 0; i < naxes; i++) {
        char * t = cpl_sprintf("%s%zd%s", soffset, offsets ? offsets[i] : 0,
                               i == naxes - 1 ? ")" : ",");
        cpl_free(soffset);
        soffset = t;
...
static void test_basic(void)
{
    ...
    cpl_msg_info(cpl_func, "multiiter allowempty test");
    {
        hdrl_iter * subiters[] = {
            hdrl_frameiter_new(frames, HDRL_ITER_OWNS_DATA, 2,
                               (intptr_t[]){HDRL_FRAMEITER_AXIS_FRAME,
                                            HDRL_FRAMEITER_AXIS_EXT},
                               (intptr_t[]){0, 1}, (intptr_t[]){1, 1}, NULL),
            hdrl_frameiter_new(frames, HDRL_ITER_OWNS_DATA, 1,
                               (intptr_t[]){HDRL_FRAMEITER_AXIS_EXT},
                               (intptr_t[]){1}, NULL, NULL)
        };
        hdrl_iter  * it = hdrl_multiiter_new(2, subiters,
                                             HDRL_ITER_ALLOW_EMPTY);
        cpl_test_nonnull(it);
        int cnt = 0;
        for (hdrl_frameiter_data ** h = hdrl_iter_next(it); h != NULL;
             h = hdrl_iter_next(it)) {
