...
void hdrl_fringe_compute_test_input(void)
{
    ...
    hdrl_imagelist_unset(ilist_fringe, 1);
    hdrl_imagelist_unset(ilist_fringe, 0);
    hdrl_fringe_compute(ilist_fringe, ilist_obj, stat_mask, collapse_params,
                         &master, &contrib_map, NULL);
    cpl_test_error(CPL_ERROR_NULL_INPUT);
    cpl_test_null(master);
...
void hdrl_fringe_correct_test_input(void)
{
    ...
    /* now we add real data: two frames with Poisson noise centred
     * at different levels
     */
    double* pdata = cpl_image_get_data_double(hdrl_image_get_image(hima1));
    int size = hdrl_image_get_size_x( hima1 ) * hdrl_image_get_size_y( hima1 );
    hdrl_random_state * rng = hdrl_random_state_new(1, NULL);
    for(int i=0;i<size;i++) {
        pdata[i] = hdrl_random_poisson(rng, 100.);
    }
    pdata = cpl_image_get_data_double(hdrl_image_get_image(hima2));
    for(int i=0;i<size;i++) {
        pdata[i] = hdrl_random_poisson(rng, 200.);
    }
...
void hdrl_mime_fringe_amplitudes_test(void)
{
    ...
    cpl_test_assert(cpl_mask_not(mask) == CPL_ERROR_NONE);
