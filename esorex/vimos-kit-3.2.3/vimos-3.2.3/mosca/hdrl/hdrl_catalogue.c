...
#include "catalogue/casu_utilfunctions.h"
...
hdrl_catalogue_parameter_verify(const hdrl_parameter    *   param)
{
    ...
    const hdrl_catalogue_parameter * param_loc = (hdrl_catalogue_parameter *)param ;
        ...
        hdrl_parameter  *   par_)
{
    ...
    hdrl_catalogue_parameter * par = (hdrl_catalogue_parameter*)par_;
...
{
    int status = CASU_OK;
    casu_fits *inf,*inconf = NULL;
    hdrl_imcore_result * intres = NULL;
    ...
    if (cpl_image_get_type(image) != CPL_TYPE_FLOAT) {
        image = cpl_image_cast(image, CPL_TYPE_FLOAT);
    }

    inf = casu_fits_wrap((cpl_image *)image, NULL, NULL, NULL);
    ...
    if (cpl_image_get_bpm_const(image) != NULL) {
        if (confidence_map) {
            conf_ = cpl_image_cast(confidence_map, CPL_TYPE_FLOAT);
        }
        else {
        ...
        cpl_image_accept_all(conf_);
             ...
             cpl_image_get_type(confidence_map) != CPL_TYPE_FLOAT) {
        conf_ = cpl_image_cast(confidence_map, CPL_TYPE_FLOAT);
    }
    inconf = casu_fits_wrap((cpl_image *)conf_, NULL, NULL, NULL);

    /* Run imcore */
    res = cpl_calloc(sizeof(*res), 1);
    intres = cpl_calloc(sizeof(*intres), 1);
    hdrl_casu_imcore(inf, inconf, wcs,
                ...
                param->det_saturation, intres, &status);

    if (intres->catalogue) {
        res->catalogue = cpl_table_duplicate(casu_tfits_get_table(intres->catalogue));
        res->qclist = cpl_propertylist_duplicate(casu_tfits_get_ehu(intres->catalogue));
...
cleanup:
    ...
    if (intres) {
        casu_tfits_delete(intres->catalogue);
    }
