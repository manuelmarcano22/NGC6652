...
# add option to enable gcc and clangs thread sanitizer, needs sanitized
# libgomp, cpl and esorex to be effective. These can be retrieved from jenkins.
#---------------------------
AC_DEFUN([ESO_ENABLE_TSAN],
[
    AC_ARG_ENABLE(tsan,
                  AS_HELP_STRING([--enable-tsan],
                                 [compiles with thread sanitizer, may need
                                 sanitized libgomp, cpl and esorex to be effective.
                                 [[default=$1]]]),
                  eso_enable_tsan=$enableval, eso_enable_tsan=$1)
    
    if test x"$eso_enable_tsan" = xyes; then
        ESO_PROG_CC_FLAG([fsanitize=thread], [CFLAGS="$CFLAGS -fsanitize=thread"
                                     LDFLAGS="$LDFLAGS -fsanitize=thread"])
    fi
])

# ESO_ENABLE_ASAN(asan=no)
# add option to enable gcc and clangs address sanitizer, needs sanitized
# cpl to be effective. This can be retrieved from jenkins.
# Note that valgrind does more than asan, but slower
#---------------------------
AC_DEFUN([ESO_ENABLE_ASAN],
[
    AC_ARG_ENABLE(asan,
                  AS_HELP_STRING([--enable-asan],
                                 [compiles with address sanitizer, may need
                                 sanitized cpl to be effective.
                                 [[default=$1]]]),
                  eso_enable_asan=$enableval, eso_enable_asan=$1)
    
    if test x"$eso_enable_asan" = xyes; then
        ESO_PROG_CC_FLAG([fsanitize=address], [CFLAGS="$CFLAGS -fsanitize=address"
