    ...
    if (wcshdo(0,wp, &i, &o)) {
        ...
        *olist = cpl_wcs_fitsstr2plist(outstr);
        ...
        for (i = 2; i > 0; --i) {
            ...
            for (j = 2; j > 0; --j)
            {
                char *cd = cpl_sprintf("CD%1d_%1d", i, j);
                cpl_property *_p = cpl_property_new(cd,
                                                    CPL_TYPE_DOUBLE);

                cpl_property_set_double(_p,
                                        cpl_matrix_get(wcs->cd, i - 1, j - 1));
                cpl_property_set_comment(_p, "Coordinate transformation matrix "
                                         "element");

                cpl_free(cd);

                cpl_propertylist_insert_after_property(*olist, "CRPIX2", _p);
                cpl_property_delete(_p);
