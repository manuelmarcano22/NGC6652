                                             ...
                                             const cpl_image * lab)
{
    ...
    bpm = cpl_image_get_bpm_const(self);
    ...
    naperts = (cpl_size)cpl_image_get_max(labuse);
    if (naperts <= 0) {
    ...
    sqsum  = cpl_calloc((size_t)naperts, sizeof(double));
    ...
    for (j = 0; j < ny; j++) {
        for (i = 0; i < nx; i++) {
            ...
            if (aperts->min_val[k] <= 0) {
                ...
                aperts->xcentroid[k]+=(double)(i+1)*(pix-aperts->min_val[k]+ep);
                aperts->ycentroid[k]+=(double)(j+1)*(pix-aperts->min_val[k]+ep);
            } else {
                /* Accumulate weighted positions */
    ...
    for (k = 0; k < aperts->naperts; k++) {
        ...
        if (npix > 1) {
            aperts->stdev[k] = sqrt(aperts->varsum[k]/((double)npix-1.0));
        } else {
            aperts->stdev[k] = -1.0;
        }
    ...
    for (k = 0; k < naperts; k++) {
        med_array = cpl_malloc((size_t)aperts->npix[k] * sizeof(double));
        count=0;
        for (j = 0; j < ny; j++) {
            for (i = 0; i < nx; i++) {
                if (plab[i+j*nx]==(k+1)) med_array[count++] = pd[i+j*nx];
            }
        }
        aperts->median[k] = cpl_tools_get_median_double(med_array, count);
        cpl_free(med_array);
    }
