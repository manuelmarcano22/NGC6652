...
int main(void)
{
    ...
    for (itype = 0; itype < sizeof(img_types)/sizeof(img_types[0]); itype++) {
        ...
        cpl_test_null( cpl_stats_new_from_image(img, CPL_STATS_MIN ||
                                                CPL_STATS_MAX) );
        cpl_test_error(CPL_ERROR_INVALID_TYPE);

        cpl_test_null( cpl_stats_new_from_image(img, ~CPL_STATS_ALL) );
        cpl_test_error(CPL_ERROR_UNSUPPORTED_MODE);

        cpl_test_null( cpl_stats_new_from_image_window(img, CPL_STATS_ALL, 2, 2,
                                                       1, 1) );
        cpl_test_error(CPL_ERROR_ILLEGAL_INPUT);

        cpl_test_null( cpl_stats_new_from_image_window(img, CPL_STATS_ALL, 1, 1,
                                                       IMAGE_SIZE_X,
                                                       IMAGE_SIZE_Y+1) );
        cpl_test_error(CPL_ERROR_ACCESS_OUT_OF_RANGE);

        cpl_test_null( cpl_stats_new_from_image_window(img, CPL_STATS_ALL, 0, 0,
                                                       IMAGE_SIZE_X,
                                                       IMAGE_SIZE_Y) );
        cpl_test_error(CPL_ERROR_ACCESS_OUT_OF_RANGE);

        cpl_test_zero( cpl_image_fill_noise_uniform(img, (double)-IMAGE_SIZE_Y,
                                                    (double)IMAGE_SIZE_X) );

        cpl_test_nonnull( statall = cpl_stats_new_from_image(img,
                                                             CPL_STATS_ALL) );
        ...
        error = cpl_stats_dump(statall, CPL_STATS_MIN || CPL_STATS_MAX, stream);
        cpl_test_eq_error(error, CPL_ERROR_ILLEGAL_INPUT);
        ...
        for (istat = 0; istat < sizeof(istats)/sizeof(istats[0]); istat++) {
            ...
            cpl_test_zero( cpl_stats_dump(statone, istats[istat], stream) );
